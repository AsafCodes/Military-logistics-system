...
def mock_matrix_db(db_session):
    ...
    users["company_cmdr_b"] = models.User(
        ...
        unit_hierarchy="188/53/B"  # mirrors company_cmdr_a, which had one
    )
    ...
    # unit_hierarchy is the MATERIALISED path ("188/53/B"), not the unit_path
    # style ("Brigade1/Bat1/Co_B") this fixture originally used here. Scoping in
    # dependencies.scope_equipment_query() prefix-matches the numeric path, so
    # the two formats never match and the commander-visibility assertions failed
    # on seed data rather than on a real defect. (The two columns coexisting is
    # DATA-M16; not fixed here.)
    items.append(models.Equipment(
        catalog_item_id=cat.id, status="Functional", unit_hierarchy="188/53/B",
