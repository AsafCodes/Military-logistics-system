...
class TestRBACMatrix:
    ...
    @pytest.mark.xfail(
        reason=(
            "SEC-H3: routers/equipment.py:125-128 grants transfer rights if the "
            "profile NAME appears in a hardcoded allowlist, in addition to the "
            "permission column. 'Company Tech Soldier' is in that list, so the "
            "403 gate is bypassed and the request reaches the equipment lookup "
            "(404 for the nonexistent id below). Brigade and Battalion techs "
            "are NOT in the list and do return 403, so only the third assertion "
            "fails. Delete this marker when SEC-H3 removes the name allowlist."
        ),
        strict=False,
    )
    ...
    @pytest.mark.xfail(
        reason=(
            "SEC-H6: routers/maintenance.py:90 resolves the target equipment by "
            "raw id with an existence check only -- no unit scope, no ownership "
            "check. The coarse can_change_maintenance_status boolean is checked "
            "and Company Tech Soldier has it, so Co A's tech repairs Co B's item "
            "and gets 200. dependencies.get_scoped_equipment_or_404() already "
            "exists for exactly this and is not used here. Note this failure was "
            "previously MASKED: the test posted to '/maintenance/fix' (no id), "
            "which matches no route, so the 403/404 assertion passed on a routing "
            "miss. Delete this marker when SEC-H6 scopes the write paths."
        ),
        strict=False,
    )
    def test_company_tech_cannot_fix_other_company_item(self, client: TestClient, token_company_tech, token_bat_cmdr):
        ...
        # The original test posted to "/maintenance/fix" with no id, which
        # matches no route -- so the 403/404 assertion below passed vacuously
        # on a routing miss rather than on scope enforcement.
        res = client.post(f"/maintenance/fix/{item_b['id']}", json={"equipment_id": item_b["id"], "notes": "Hacking!"}, headers=token_company_tech)
