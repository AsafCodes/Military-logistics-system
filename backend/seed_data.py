...
def seed_matrix():
    ...
    u_master = models.User(personal_number="u_master", full_name="Master Admin", password_hash=pw, role=models.UserRole.MASTER, profile_id=p_master.id, unit_hierarchy=None)
    u_brig_cmdr = models.User(personal_number="u_brig_cmdr", full_name="Brigade Commander", password_hash=pw, role=models.UserRole.MANAGER, profile_id=p_brigade_tech_cmdr.id, unit_hierarchy="188")
    u_bn_cmdr = models.User(personal_number="u_bn_cmdr", full_name="Battalion Commander", password_hash=pw, role=models.UserRole.MANAGER, profile_id=p_bat_tech_cmdr.id, unit_hierarchy="188/53")
    u_tech_bat = models.User(personal_number="u_tech_bat", full_name="Bat Tech Soldier", password_hash=pw, role=models.UserRole.TECHNICIAN, profile_id=p_bat_tech_soldier.id, unit_hierarchy="188/53")
    u_co_cmdr_a = models.User(personal_number="u_co_cmdr_a", full_name="Commander Co A", password_hash=pw, role=models.UserRole.MANAGER, profile_id=p_company_cmdr.id, unit_hierarchy="188/53/A")
    u_co_cmdr_b = models.User(personal_number="u_co_cmdr_b", full_name="Commander Co B", password_hash=pw, role=models.UserRole.MANAGER, profile_id=p_company_cmdr.id, unit_hierarchy="188/53/B")
    u_soldier = models.User(personal_number="u_soldier", full_name="Simple Soldier", password_hash=pw, role=models.UserRole.USER, profile_id=p_soldier.id, unit_hierarchy="188/53/A")

    users.extend([u_master, u_brig_cmdr, u_bn_cmdr, u_tech_bat, u_co_cmdr_a, u_co_cmdr_b, u_soldier])
    db.add_all(users)
    db.commit()
