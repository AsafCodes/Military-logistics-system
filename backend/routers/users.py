...
def create_user(user: schemas.UserCreate, db: Session = Depends(get_db)):
    if (
        ...
        .filter(models.User.personal_number == user.personal_number)
        .first()
    ):
    ...
    profile = (
        ...
        .filter(models.Profile.name == target_profile_name)
        .first()
    )
...
def promote_user(
    ...
    target_user = (
        db.query(models.User).filter(models.User.id == req.target_user_id).first()
    )
    if not target_user:
...
def get_my_equipment(
    ...
    items = (
        ...
        .filter(models.Equipment.holder_user_id == current_user.id)
    ...
    return [
        schemas.EquipmentResponse(
            ...
            compliance_level=get_daily_status(item.last_verified_at),
...
def list_all_users(
    ...
    if q:
        query = query.filter(
            ...
            | (models.User.personal_number.ilike(f"%{q}%"))
        )
    else:
