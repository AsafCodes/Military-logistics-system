...
def create_equipment(
    ...
    cat_item = (
        ...
        .filter(models.CatalogItem.name == item.catalog_name)
        .first()
    )
    if not cat_item:
    ...
    new_item = models.Equipment(
        catalog_item_id=cat_item.id, serial_number=item.serial_number
    )
    db.add(new_item)
    db.commit()
    db.refresh(new_item)
...
def assign_owner(
    ...
    allowed_tech_roles = [
        "Company Tech Soldier",
        "Battalion Tech Commander",
        "Brigade Tech Commander",
    ]
    has_permission = (
        (current_user.profile and current_user.profile.can_change_assignment_others)
        or (current_user.profile and current_user.profile.name in allowed_tech_roles)
        or current_user.role == "master"
    )

    if not has_permission:
        raise HTTPException(
            status_code=403, detail="Permission denied. Profile cannot assign owners."
        )

    item = (
        db.query(models.Equipment)
        .filter(models.Equipment.id == req.equipment_id)
        .first()
    )
    if not item:
...
def transfer_equipment(
    ...
    if not has_permission:
        raise HTTPException(
            status_code=403, detail="Permission Denied: Cannot transfer equipment."
        )

    # XOR Validation
    if req.to_holder_id is None and req.to_location is None:
        raise HTTPException(
            status_code=400, detail="Must provide either to_holder_id or to_location."
        )
    if req.to_holder_id is not None and req.to_location is not None:
        raise HTTPException(
            status_code=400, detail="Cannot transfer to both Person and Location."
        )
    ...
    try:
        if req.to_holder_id:
            target = (
                db.query(models.User).filter(models.User.id == req.to_holder_id).first()
            )
            if not target:
        ...
        raise HTTPException(
            status_code=500, detail=f"Internal Server Error during transfer: {str(e)}"
        ) from e
...
def verify_equipment_daily(
    ...
    if item.holder_user_id != current_user.id:
        raise HTTPException(
            status_code=403,
            detail="Permission Denied: You can only verify equipment you hold.",
        )
