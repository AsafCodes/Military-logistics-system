...
def report_fault(
    ...
    fault_type = (
        db.query(models.FaultType).filter(models.FaultType.name == fault_name).first()
    )
    if not fault_type:
        is_manager = (
            current_user.profile and current_user.profile.can_change_maintenance_status
        )
        fault_type = models.FaultType(
            name=fault_name, is_pending=not is_manager, requested_by_id=current_user.id
        )
        db.add(fault_type)
        db.commit()
...
def fix_equipment(
    ...
    if not (
        ...
        if current_user.role != "master":
            raise HTTPException(
                status_code=403, detail="Permission denied: Cannot fix equipment"
            )
    ...
    db.query(models.MaintenanceLog).filter(
        ...
        {"status": "Closed", "closed_at": datetime.utcnow()}, synchronize_session=False
    )
